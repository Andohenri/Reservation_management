...
import { capitalized, setSatus, subtract } from '../utils/utils'
        ...
        <div className='p-4 shadow'>
          <div className='flex_between border-b pb-4'>
            ...
            <div className='flex gap-4 flex-col text'>
              ...
              <p className='flex font-semibold gap-2'><FaChair size={24} /> {trip?.trainId.capacity}</p>
            </div>
          </div>
          <div className='p-4 flex flex-col gap-6 text'>
            ...
            <div className='flex_between'>
              <span>{capitalized(trip?.origin)}</span>
              <span>{capitalized(trip?.destination)}</span>
            </div>
            <div className='flex_between gap-4 font-bold text-3xl'>
              <span className=''>{trip?.origin.slice(0, 3).toUpperCase()}</span>
              <span className={`hidden sm:flex relative flex-1 ${trip?.status === 'pending' ? 'justify-start' : trip?.status === 'in progress' ? 'justify-center' : 'justify-end'} before:content-[""] before:absolute before:w-full before:bg-gray-800 before:h-0.5 before:bottom-0 before:z-0`}>
                <WiTrain size={32} className='text-indigo-500 z-10' />
              </span>
            ...
            <div className='flex_between text-gray-600 font-semibold'>
              ...
              <span className='flex_between gap-1'><MdAirlineSeatReclineNormal size={24} />Restants: {trip?.avalaible_seats}</span>
            </div>
                ...
                {trip.avalaible_seats > 0 && (
                  <>
                    <div className='flex items-center gap-2'>
                      <h4 className='font-semibold'>Nombre de billets: </h4>
